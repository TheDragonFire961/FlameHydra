<?php
/*
    migrate.php: the migration assistant for RethCommentsring.
...
if ($cminInstallerStage === "database" && $cminSubmit === true && !empty($_POST["databasetype"])) {
    ...
    if ($_POST["databasetype"] === "mysqli") {
        if (!empty($_POST["databasename"]) && !empty($_POST["databaseserver"]) && !empty($_POST["databaseserverport"]) && !empty($_POST["databasename"]) && !empty($_POST["databaseprefix"]) && !empty($_POST["databasename"]) && !empty($_POST["databasepassword"])) { 
            ...
            header("Location: migrate.php?page=settings");
        } else {
            header("Location: migrate.php?page=database&type=mysqli");
        }
    } if ($_POST["databasetype"] === "mysqli") {
        ...
        if (!empty($_POST["databasefile"])) {
            ...
            header("Location: migrate.php?page=database&type=sqlite");
        }
    } 
}
...
if ($cminInstallerStage === "lang") {
    echo '<form action="migrate.php?page=lang&amp;submit=1" method="post">';
...
// Welcome to RethCommentsring
    ...
    if ($cminInstallerLanguage === "en-au" || $cminInstallerLanguage === "en-ca" || $cminInstallerLanguage === "en-gb" || $cminInstallerLanguage === "en-us") {
        ...
        echo 'RethCommentsring is a PHP application used to provide an interactive, responsive comments space.<br>This installer is designed to help you migrate from an existing comments solution to RethCommentsring. It is not for creating a new RethCommentsring installation from scratch; for this, see <a href="index.php?page=lang">the main installer</a>.';
        ...
        echo '<p>To read the full GNU General Public Licence 3.0, click <a href="gpl3.txt" target="_blank">here</a>.';
        echo '</div>';
        ...
        if (!empty($cminRequiredCheck) && $cminRequiredCheck === true && !empty($cminDatabaseCheck) && !empty($cminRandomBytesGenerationCheck)) {
            ...
            echo '<a href="migrate.php?page=lang" class="button red">Previous</a> | ';
            echo '<a href="migrate.php?page=olddatabase" class="button green">Next</a>';
        } else {
            echo '<p class="check-fail center">You cannot proceed with the installation because one or more required checks failed.</p>';
            echo '<p class="center"><a href="migrate.php?page=lang" class="button red">Previous</a></p>';
        }
        echo '</p>';
    }
} else if ($cminInstallerStage === "olddatabase") {
    if ($cminInstallerLanguage === "en-au" || $cminInstallerLanguage === "en-ca" || $cminInstallerLanguage === "en-gb" || $cminInstallerLanguage === "en-us") {
        ...
        echo '</ul>';
        echo '<form action="migrate.php?page=database&submit=1" method="post">';
        if (extension_loaded("mysqli")) {
        ...
        echo '</div>';
        echo '<p class="center"><a href="migrate.php?page=welcome" class="button red">Previous</a>';
        ...
        echo '        document.getElementById("database-params").innerHTML = '."'".'Database server name<br><input type="text" class="input-box" name="olddatabaseserver" value="localhost"><br>'."'".';';
        echo '        document.getElementById("database-params").innerHTML += '."'".'Database server port<br><input type="text" class="input-box" name="olddatabaseport" value="3306"><br>'."'".';';
        echo '        document.getElementById("database-params").innerHTML += '."'".'Database name<br><input class="input-box" type="text" name="olddatabasename" value="rethcommentsring"><br>'."'".';';
        echo '        document.getElementById("database-params").innerHTML += '."'".'Database table prefix<br><input type="text" class="input-box" name="olddatabaseprefix" value="rethcommentsring_"><br>'."'".';';
        echo '        document.getElementById("database-params").innerHTML += '."'".'Database username<br><input type="text" class="input-box" name="olddatabaseusername" value="root"><br>'."'".';';
        echo '        document.getElementById("database-params").innerHTML += '."'".'Database password<br><input type="password" class="input-box" name="olddatabasepassword"><br>'."'".';';
        echo '    }';
        echo '    if (databasetype === "sqlite") {';
        echo '        document.getElementById("database-params").innerHTML = '."'".'Database file<br><input type="text" class="input-box" name="olddatabasefile" value=""><br>'."'".';';
        echo '        document.getElementById("database-params").innerHTML += '."'".'Database table prefix<br><input class="input-box" type="text" name="olddatabaseprefix" value="rethcommentsring_"><br>'."'".';';
        echo '        document.getElementById("database-params").innerHTML += '."'".'Database file is in web root <input type="checkbox" name="olddatabasefilewebroot"><br>'."'".';';
        echo '    }';
        echo '}';
        if (!empty($_GET["type"])) {
            echo 'document.getElementById("database-next").style.display = "inline"';
...
} else if ($cminInstallerStage === "newdatabase") {
    if ($cminInstallerLanguage === "en-au" || $cminInstallerLanguage === "en-ca" || $cminInstallerLanguage === "en-gb" || $cminInstallerLanguage === "en-us") {
        echo '<h2 class="header">Database Setup</h2>';
        echo 'RethCommentsring supports the following databases:';
        echo '<ul>';
    ...
    if ($cminInstallerLanguage === "en-au" || $cminInstallerLanguage === "en-ca" || $cminInstallerLanguage === "en-gb" || $cminInstallerLanguage === "en-us") {
        ...
        echo '<form action="migrate.php?page=settings&submit=1" method="post">';
    ...
    if (!empty($cminInstallerLanguage)) {
        if ($cminInstallerLanguage === "en-au" || $cminInstallerLanguage === "en-ca" || $cminInstallerLanguage === "en-gb" || $cminInstallerLanguage === "en-us") {
            ...
            echo '<p class="center"><a href="migrate.php?page=welcome" class="button red">Return</a></p>';
        } else {
            echo "<h2>No such stage</h2>";
            echo 'The stage given in the URL was not recognized.';
            echo '<p class="center"><a href="migrate.php?page=welcome" class="button red">Return</a></p>';
        ...
        echo "<h2>No such stage</h2>";
        echo 'The stage given in the URL was not recognized.';
        echo '<p class="center"><a href="migrate.php?page=lang" class="button red">Return</a></p>';
    }
}
     ?>
     <?php
